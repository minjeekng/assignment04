...
ggplot(birthrate, mapping = aes(x = Province, y = birthrate, fill = year, alpha = birthrate, shape = Province)) +
  facet_wrap(~ year) +
  coord_flip() + 
  geom_col(width = 0.5) +
...
ggplot(workhour, mapping = aes(x = workhour, fill = Agegroup, colour = Agegroup)) +
  facet_grid(year ~ Agegroup) +
